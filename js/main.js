...
function openProcedureDetailsModal(procId) {
    ...
    content.innerHTML = `
        ...
        <div class="text-sm text-gray-400 mb-4">Autor: ${proc.user_name || '-'}</div>
        ${renovacaoHtml}
        <div class="mb-4">
...
async function saveProcedure() {
    ...
    const user_name = (window.currentUser && window.currentUser.user_metadata && window.currentUser.user_metadata.name)
        ? window.currentUser.user_metadata.name
        : (window.currentUser && (window.currentUser.name || window.currentUser.email)) || 'Usuário';
    console.log('Dados para salvar:', {name, description, steps, user_id, user_name});
    if (!name) {
...
function initializeTheme() {
    ...
    if (currentTheme === 'dark') {
        ...
        document.getElementById('theme-icon').className = 'fas fa-sun mr-2';
        ...
        document.getElementById('theme-icon').className = 'fas fa-moon mr-2';
        document.getElementById('theme-text').textContent = 'Modo Escuro';
        document.getElementById('mobile-theme-icon').className = 'fas fa-moon';
    }
}
...
// Exibe info do usuário na sidebar (desktop) e atualiza botão mobile
function showSidebarUserInfo(user) {
    const sidebarUserInfo = document.getElementById('sidebar-user-info');
    if (!sidebarUserInfo) return;
    if (user) {
        // Tenta pegar o nome do user_metadata, senão mostra email
        let nome = (user.user_metadata && user.user_metadata.name) ? user.user_metadata.name : (user.name || user.email || 'Usuário');
        sidebarUserInfo.innerHTML = `
            <div class="flex flex-col items-center">
                <div class="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-2xl text-blue-600 dark:text-blue-300 mb-2">
                    <i class="fas fa-user"></i>
                </div>
                <div class="font-semibold text-gray-800 dark:text-gray-100 text-sm truncate">${nome}</div>
                <button onclick="logout()" class="mt-3 px-4 py-2 bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-800 transition-colors w-full flex items-center justify-center">
...
function loadRecentActivity() {
    ...
    const pageActivities = recentActivity.slice(start, end);

    if (pageActivities.length === 0) {
        container.innerHTML = '<p class="text-gray-500 dark:text-gray-400 text-center py-8">Nenhuma atividade recente</p>';
        return;
    }
...
function loadServicesList() {
    ...
    if (services.length === 0) {
        container.innerHTML = '<p class="text-gray-500 dark:text-gray-400 text-center py-8">Nenhum serviço cadastrado</p>';
        return;
    }
    // Paginação
...
function editService(serviceId) {
    ...
    supabaseClient.from('services').update(updateData).eq('id', serviceId).eq('user_id', user_id).then(({ error }) => {
        if (error) {
            alert('Erro ao atualizar serviço: ' + error.message);
            return;
        }
...
function deleteService(serviceId) {
    const user_id = window.currentUser ? window.currentUser.id : null;
    showConfirmModal('Tem certeza que deseja excluir este serviço?', function() {
        supabaseClient.from('services').delete().eq('id', serviceId).eq('user_id', user_id).then(({ error }) => {
            if (error) {
